    ...
    <>
      <div className="p-3 bg-white rounded flex justify-between items-center gap-2 sm:gap-3">
        <div className="flex gap-3 items-center">
          ...
          <div>
            <h2 className="color4 font-12 sm:font-14 font-normal">{data.packageName}</h2>
          </div>
        </div>
        <div>
          {/* <button
      ...
      {packages?.slice(0, n).map((item, i) => {
        return (
          <div className="mt-2" key={i}>
            <PackageItem data={item} vendorData={item} bool={bool} boolval={boolval} setorder={setorder}/>
          </div>
        );
      })}
      {packages?.length>n ?         <button 
      onClick={()=>{setn(n+2)}}
